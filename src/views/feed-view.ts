...
import {Details, ProjectReactionsInitialResponse, ProjectReactionsResponse, ReactionEmojis, ValueOf} from "../types/app";
...
export class FeedViewProvider implements WebviewViewProvider {
	...
	private reactionsTransformed: Map<WorkspaceFolder, {[reaction_group_id: string]:{
		name?: string;
			file: string;
			branch?: string;
			content?: string;
	...
	private async setReactionsTransformed(folder: WorkspaceFolder){
		...
		const reactionsTransformed:{[reaction_group_id: string]:{
			name?: string;
			file: string;
			branch?: string;
			content?: string;
	...
	public async setReactions(folder: WorkspaceFolder, projectReactions: ProjectReactionsInitialResponse['reactions']) {
		...
		this.reactions.set(folder, projectReactions.map(({ids, file_name, original_sha_line})=>({ids, file_name, original_sha: original_sha_line.split('_')[0], original_line: parseInt(original_sha_line.split('_')[1],10)})));
		await this.setReactionsTransformed(folder);
	...
	public async addReactions(folder: WorkspaceFolder, projectReactions: ProjectReactionsResponse['reactions']) {
		if(this.reactions.has(folder)){
			...
			reactions?.push(...projectReactions.map(({id, file_name, original_sha, original_line})=>({ids:[id],file_name, original_sha, original_line})));
		}else{
			this.reactions.set(folder, projectReactions.map(({id, file_name, original_sha, original_line})=>({ids:[id],file_name,original_sha, original_line})));
		}
		await this.setReactionsTransformed(folder);
	...
	private getHtmlContent(folder: WorkspaceFolder): string{
		...
		const title = `<div class="table-container" role="table" aria-label="Destinations">
		...
		return (
			...
			${title}
			  ${Object.values(reactionsTransformed).map(reaction=>{
					  return (
					  `
					  <div class="flex-table row" role="rowgroup">
						  ...
						  <div class="flex-row" role="cell">${reaction.ts?format(new Date(reaction.ts)):'...'}</div>
